...
**Status: DONE — all nine steps resolved (eight built, one retired), and the plugin left 0.x at
1.0.0** (last checked 2026-08-19, at 3.11.0). The version left the 0.x range because every step in
this plan has landed: the major digit was always the claim that the plugin is finished and worth
installing. Releases since are the plugin's own history and live in its `CLAUDE.md`, not here. All eight decisions are settled (§4) and every open question is closed
...
0.13.1 is the diagnostic that 0.13.0 made necessary. Existence gating had one reason to hide and it
was visible on the page; eligibility has six, four of them invisible - the sources' tags, the
target's own tags, the common-tags fold, the exclusion filters. An absent button went from
self-explanatory to unexplainable. `coop().debugButtons`, typed into the console, turns on a channel
in both plugins naming per button whether it is shown or hidden and why. **A gate whose reasons the
user cannot see needs a way to ask it.**

0.13.2 fixed that switch being silent in the one case it is always used in. Live paste, a day later:
the structural lines appeared and not one per-button outcome, because the outcomes fired from the
probe's callback and a probe runs once per entity — so turning the flag on while already on the page
