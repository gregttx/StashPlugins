...
| `style.test.js` | The two dialogs' shared chrome, as text rather than through a harness: both plugins' CSS strings are parsed, their `npt-` / `cpt2s-` prefixes stripped, and every selector defined on both sides has to match byte for byte. Selectors only one dialog has (the tree, the inspector, each plugin's own log-line kinds) are ignored. The modal's background is pinned by name to the grey the rest of the palette was chosen against, since the two values it drifted between were both plausible Stash greys. |
